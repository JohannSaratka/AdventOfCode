...
class Test(unittest.TestCase):
    def test_memory_manipulation(self):
        input_data = ["mask = XXXXXXXXXXXXXXXXXXXXXXXXXXXXX1XXXX0X",
                ...
                "mem[7] = 101",
                "mem[8] = 0"]
        self.assertEqual(solve(input_data), 165)
    ...
    def test_sum_two_entries(self):
        input_data = ["mask = 000000000000000000000000000000X1001X",
                ...
                "mask = 00000000000000000000000000000000X0XX",
                "mem[26] = 1"]
        self.assertEqual(solve_part_two(input_data), 208)
...
def solve(program):
    ...
    for line in program:
        if line[1] == 'a':
            ...
            set_mask, reset_mask = generate_masks(bitmask)
        else:
            match = memory_prog.search(line)
            address, value = match.groups()
            masked_value = (int(value) & reset_mask) | set_mask
            memory[address] = masked_value
    return sum(memory.values())


def generate_masks(mask):
    set_mask = 0x0000000000000000
    reset_mask = 0xffffffffffffffff
...
def solve_part_two(program):
    ...
    for line in program:
        if line[1] == 'a':
            ...
            match = memory_prog.search(line)
            address, value = match.groups()
            bitaddress = list(format(int(address), '036b'))
            floating = generate_floating(bitmask, bitaddress)
            addresses = generate_addresses(floating)
            for addr in addresses:
                memory[addr] = int(value)
    return sum(memory.values())

def generate_floating(mask, value):
    for i, bit in enumerate(reversed(mask)):
        if bit == '1':
...
def generate_addresses(floating):
    possible_adresses = [[]]
    pos = 0
    while True:
        bit = floating[pos]
        if bit == 'X':
            new_adresses = []
            for nested in possible_adresses:
                new_adresses.append(nested[:])
                new_adresses[-1].extend('0')
                new_adresses.append(nested[:])
                new_adresses[-1].extend('1')
            possible_adresses = new_adresses[:]
        else:
            for nested in possible_adresses:
                nested.append(bit)
        pos += 1
    ...
    return [int(''.join(nested), 2) for nested in possible_adresses]
