...
adapters that could connect to it directly would need to have a joltage rating
of 1, 2, or 3 jolts. Of these, only one you have is an adapter rated 1 jolt
(difference of 1).
...
class Test(unittest.TestCase):
    def test_adapters(self):
        ...
        self.assertEqual(calculate_joltage_difference(adapters), 35)

    def test_adapters_longer(self):
        adapters = [28, 33, 18, 42, 31, 14, 46, 20, 48, 47, 24, 23, 49, 45, 19,
                    38, 39, 11, 1, 32, 25, 35, 8, 17, 7, 9, 4, 2, 34, 10, 3]
        self.assertEqual(calculate_joltage_difference(adapters), 220)

    def test_find_possibilities(self):
        adapters = [16, 10, 15, 5, 1, 11, 7, 19, 6, 12, 4]
        self.assertEqual(calculate_possibilities(adapters), 8)

    def test_find_possibilities_longer(self):
        adapters = [28, 33, 18, 42, 31, 14, 46, 20, 48, 47, 24, 23, 49, 45, 19,
                    38, 39, 11, 1, 32, 25, 35, 8, 17, 7, 9, 4, 2, 34, 10, 3]
        self.assertEqual(calculate_possibilities(adapters), 19208)


def get_diff_list(adapters):
    adapters.sort()
    adapters.insert(0, 0)
    adapters.append(max(adapters) + 3)
    return [value - adapters[num] for num, value in enumerate(adapters[1:])]


def calculate_joltage_difference(adapters):
    diff = get_diff_list(adapters)
    return diff.count(1) * (diff.count(3))


def solve(adapters):
    adapters = list(map(int, adapters))
    return calculate_joltage_difference(adapters)


def calculate_possibilities(adapters):
    arrangements = list()
    # possibilities: 11=2, 111=4 1111=7 11111=13 111111=24
    # see tribonacci series
    lookup = {1: 1, 2: 2, 3: 4, 4: 7, 5: 13, 6: 24}

    # get diff list
    diff = get_diff_list(adapters)
    # count consecutive ones with groupby
    for key, group in itertools.groupby(diff):
        if key == 1:
            arrangements.append(lookup[len(list(group))])

    # multiply all
    return functools.reduce(lambda x, y: x * y, arrangements)


def solve_part_two(adapters):
    adapters = list(map(int, adapters))
    return calculate_possibilities(adapters)
