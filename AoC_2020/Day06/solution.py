...
class Test(unittest.TestCase):
    ...
    def test_anyone_yes_answer(self):
        self.assertEqual(solve(self.group_answers), 11)

    def test_everyone_yes_answer(self):
        self.assertEqual(solve_part_two(self.group_answers), 6)
...
def solve(answers):
    ...
    # add last complete group to list
    groups.append(group)
    return sum([len(g) for g in groups])


def solve_part_two(answers):
    groups = list()
    group = list()
    for line in answers:
        if not line:
            groups.append(group[0].intersection(*group[1:]))
            group = []
        else:
