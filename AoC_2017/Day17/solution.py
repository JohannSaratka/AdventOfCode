...
def solve(step):
    ...
    for i in xrange(1,2018):
        next_pos = (current_pos + step) % len(spinlock)
        spinlock.insert(next_pos + 1, i)
        current_pos = next_pos + 1
    return spinlock[current_pos + 1]
        
    
def solvePartTwo(step):
    spinlock_length = 1
    current_pos = 0
    val_after_zero = 0
    for i in xrange(1,50000000):
        next_pos = (current_pos + step) % spinlock_length
        if next_pos == 0:
            val_after_zero = i
        spinlock_length += 1
        current_pos = next_pos + 1
