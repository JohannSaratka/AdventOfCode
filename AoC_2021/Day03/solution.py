...
class Test(unittest.TestCase):
    ...
    def test_power_consumption(self):
        self.assertEqual(solve(self.diagnostic), 198)

    def test_life_support_rating(self):
        self.assertEqual(solve_part_two(self.diagnostic), 230)


def bin_digits_to_int(digits):
    return sum(c << i for (i, c) in enumerate(digits[::-1]))


def solve(diagnostic):
    most_common = [0] * len(diagnostic[0])
    for value in diagnostic:
        bits = [int(x) for x in list(value)]
        most_common = list(map(operator.add, most_common, bits))

    digits_gamma = [int((b * 2) > len(diagnostic)) for b in most_common]
    digits_epsilon = [1 if b == 0 else 0 for b in digits_gamma]

    gamma = bin_digits_to_int(digits_gamma)
    epsilon = bin_digits_to_int(digits_epsilon)
    return gamma * epsilon


def get_oxygen_rating(diagnostic_bits):
    pos = 0
    while (len(diagnostic_bits) > 1 and
           pos < len(diagnostic_bits[0])):
        count = 0
        for value in diagnostic_bits:
            count += value[pos]
        most_common = int((count * 2) >= len(diagnostic_bits))
        diagnostic_bits = [i for i in diagnostic_bits if i[pos] == most_common]
        pos += 1
    return bin_digits_to_int(diagnostic_bits[0])


def get_scrubber_rating(diagnostic_bits):
    pos = 0
    while (len(diagnostic_bits) > 1 and
           pos < len(diagnostic_bits[0])):
        count = 0
...
def solve_part_two(diagnostic):
    diagnostic_bits = [[int(x) for x in list(value)] for value in diagnostic]

    return get_oxygen_rating(diagnostic_bits) * \
        get_scrubber_rating(diagnostic_bits)
