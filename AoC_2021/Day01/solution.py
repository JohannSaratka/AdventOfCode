...
class Test(unittest.TestCase):
    ...
    def test_count_increase(self):
        self.assertEqual(solve(self.sweep), 7)

    def test_sum_three_entries(self):
        self.assertEqual(solve_part_two(self.sweep), 5)


def get_depth(report):
    return sum([val > report[ind] for ind, val in enumerate(report[1:])])


def solve(sonar_sweep_report):
    report = list(map(int, sonar_sweep_report))
    return get_depth(report)


def solve_part_two(sonar_sweep_report):
    report = list(map(int, sonar_sweep_report))
    window = [sum(report[i:i + 3]) for i in range(len(report))]
    return get_depth(window)
